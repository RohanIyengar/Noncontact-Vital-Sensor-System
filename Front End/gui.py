...
#initialize global variables for realtime
count = 0
heartRate = 10
respRate = 10
...
#custom error check function, can do the same for other datatypes ie. float, double
def int_error_check(i):
    try:
        return int(i) # return int(i, 16)
    except ValueError:
        logger.error("An integer conversion errored with empty string: " + str(i == ""))
        return 0

# thread function to update the signals received and time values in realtime, still receives entire string
def receive_realtime(in_time=10):
    global timed_out, time_vals, signal_data
    signal_data = []
    time_vals = np.array([])
    timed_out = False
    ser_obj = com.initializeSerial() # in_port="COM4"
    last_time = 0
    ...
    while not timed_out:
        if ser_obj.inWaiting() > 0:
            ...
            # if len(new_msg) == 12:
            #     
            num_msg = [int_error_check(i) for i in new_msg] #new_msg(0:5)
            #num_msg2 = [int_error_check(i) for i in new_msg(6:11)]
            signal_data = np.append(signal_data, num_msg)
            #signal_data2 =  np.append(signal_data2, num_msg2)
            time_vals = np.linspace(0, end_time - start_time, num=len(signal_data))
            line = line + new_data
    return line

# controls receiving and makes sure the final result is stored in a csv file
def dataCall():
    ...
    data = zip(time_vals, signal_data) # , signal_data2
...
# Does the graphing on the last screen, modify to add/remove graphs as shown below.
def update_page_two():
    ...
    fig = plt.figure()
    b = fig.add_subplot(313) #b = fig.add_subplot(212)
    c = fig.add_subplot(312) #c = fig.add_subplot(211)
    d = fig.add_subplot(311) #remove once ADC works

    b.set_title ("Heart Beat Transient", fontsize=16)
    c.set_title ("Respiration Transient", fontsize=16)
    d.set_title ("Filtered Raw Signal", fontsize=16) # Don't need this once ADC works
...
def do_a_plot():
    ...
    if (count == 0):
        fig = plt.figure()
        a = fig.add_subplot(111) #a = fig.add_subplot(211)  
        fig.tight_layout()
        count+=1
